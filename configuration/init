...
project_info() {
	...
	variables() {
		...
		[ "$PRJ_CONF" ] || PRJ_CONF="$(dirname "$(find_first --target "toolchain.toml")")"
		[ "$PRJ_CONF" ] || PRJ_CONF="$(dirname "$(find_first --type dir --target "configuration")")"

		#| Project Bins Directory
		PRJ_BIN="$(dirname "$(find_first --target "init*")")"
		PRJ_RELEASE="$PRJ_ROOT/target/release"
		PATH="$PRJ_RELEASE:$PRJ_BIN:$PATH"
