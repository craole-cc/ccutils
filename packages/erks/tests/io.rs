use erks::{Code, Context, Severity, io::Error};

#[test]
fn test_file_system_error() {
  let error = Error::file_system("Disk is full");
...
fn test_network_error() {
  let error = Error::network("Connection timed out");
...
fn test_permission_error() {
  let error = Error::permission("Read access denied");
...
fn test_not_found_with_paths() {
  ...
  let error = Error::not_found_with_paths("Config file", "config", paths);
  let meta = error.metadata().unwrap();
