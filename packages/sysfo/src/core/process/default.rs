...
impl Info {
  pub fn new(system: &System) -> Self {
    ...
    let path = match process.exe() {
      ...
      None => return Self::default() /* TODO: This should return an error:
                                      * "Could not get process path" */
    };
    ...
    // // Detect shell by traversing parent processes
    // let shell =       // First find the shell process
    // for (pid, name) in dependencies.iter().rev() {
    //     let name = name.to_lowercase();
    //     let kind = match name.as_str() {
    //         n if n.contains("pwsh") || n.contains("powershell") =>
    // shell::Kind::PowerShell,         n if n.contains("bash") =>
    // shell::Kind::Bash,         n if n.contains("zsh") =>
    // shell::Kind::Zsh,         n if n.contains("fish") =>
    // shell::Kind::Fish,         n if n.contains("nu") =>
    // shell::Kind::Nushell,         n if n.contains("cmd.exe") =>
    // shell::Kind::CommandPrompt,         _ => continue,
    //     };

    //     // Once we find a shell, construct a shell::Info for it
    //     if let Some(process) = system.process(Pid::from_u32(*pid)) {
    //         shell::Info {
    //             id: *pid,
    //             name: process.name().to_string_lossy().to_string(),
    //             path:process.exe().map_or_else(PathBuf::new, |p|
    // p.to_path_buf()),             conf: shell::get_config_paths(&kind),
    //             version: shell::get_version(&kind),
    //         };
    //     }
    // }

    // // Return default if no shell is found
    // shell::Info::default();
