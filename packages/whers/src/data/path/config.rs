...
impl PathType {
  pub fn matches(&self, path: &Path) -> Result<bool, PathTypeError> {
    match self {
      Self::Any => Ok(true),
      Self::Empty => Ok(path.metadata()?.len() == 0),
      Self::Executable => Self::is_executable(&path.metadata()?),
      _ => {
        let metadata = path.metadata()?;
        let file_type = metadata.file_type();
        Ok(match self {
          Self::BlockDevice => Self::is_block_device(&metadata),
          Self::CharacterDevice => Self::is_char_device(&metadata),
  ...
  fn is_executable(metadata: &Metadata) -> Result<bool, PathTypeError> {
    #[cfg(unix)]
    {
