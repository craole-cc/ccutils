...
impl Workspace {
  ...
  pub fn execute_command(&self, cli: &Cli) -> Result<()> {
    ...
    match cli.effective_command() {
      ...
      Command::Install { crates, mode } => {
        ...
        installer.install_crates(&target_crates, &mode, cli.force, cli.verbose)?;
      }
      Command::BuildInstall { crates, mode } => {
        ...
        let to_rebuild = builder.filter_outdated_crates(&target_crates, cli.force, cli.verbose)?;

        if to_rebuild.is_empty() {
          println!("All specified binary crates are up to date. Nothing to do.");
          return Ok(());
        }
      ...
      Command::List {
        ...
        lister.list_crates(&self.members, &self.binaries, detailed, bins_only, libs_only)?;
      }
    }
  ...
  fn filter_target_crates(&self, specified_crates: &[String]) -> Result<Vec<String>> {
    ...
    loop {
      ...
      if cargo_toml.exists() {
        let content = read_to_string(&cargo_toml)
          .with_context(|| format!("Failed to read Cargo.toml at '{}'", cargo_toml.display()))?;
  ...
  fn extract_workspace_name(cargo_toml: &Path) -> Result<String> {
    let content = read_to_string(cargo_toml)
      .with_context(|| format!("Failed to read workspace Cargo.toml at '{}'", cargo_toml.display()))?;

    let parsed: toml::Value = content
      .parse()
      .with_context(|| format!("Failed to parse workspace Cargo.toml at '{}'", cargo_toml.display()))?;
    ...
    let workspace_dir = cargo_toml.parent().context("Failed to get workspace directory")?;
  ...
  fn find_binary_crates(members: &[String]) -> Result<Vec<String>> {
    ...
    for member in members {
      ...
      let member_toml = read_to_string(&cargo_path)
        .with_context(|| format!("Failed to read member Cargo.toml at '{}'", cargo_path.display()))?;

      let member_parsed: toml::Value = member_toml
        .parse()
        .with_context(|| format!("Failed to parse member Cargo.toml at '{}'", cargo_path.display()))?;

      let has_explicit_bin_target = member_parsed.get("bin").and_then(|v| v.as_array()).is_some();
      let is_binary_by_convention = Path::new(member).join("src/main.rs").exists();
