...
fn parse_rust_decimal<T: ToString>(input: T) -> Result<SmallDecimal, Error<'static>> {
  ...
  if let Ok(i) = input_str.parse::<i64>() {
    let decimal = SmallDecimal::from(i);
  ...
  match SmallDecimal::from_str(&cleaned) {
    Ok(result) => {
...
#[inline]
pub fn parse_decimal<T: ToString>(input: T) -> Result<Decimal, Error<'static>> {
  let input_str = input.to_string();
