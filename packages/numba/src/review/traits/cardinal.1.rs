...
fn cardinal_words_from_numeric(value: f64) -> String {
  // Convert the input f64 to a string to split it into integer and fractional
  // parts
  ...
  if parts.len() > 1 && !parts[1].is_empty() {
    ...
    let trimmed_fractional_part = fractional_part.trim_end_matches('0');
    ...
    if !trimmed_fractional_part.is_empty() {
      ...
      return format!("{} point {}", integer_worded, fractional_worded);
    }
  }
