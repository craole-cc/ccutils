...
impl Value {
    ...
    match value {
      Self::Words(ref input) => {
        ...
        if let Some(int_value) = ordinal_symbolic(words) {
          return Ok(Self::OrdinalSymbolic(int_value, words.to_string()));
        }
        ...
        if let Some((percent, _)) = percentage_symbol(words) {
          return Ok(Self::PercentageSymbol(percent, words.to_string()));
        }

        // Try parsing as percentage words
        if let Some((percent, base)) = percentage_worded(words) {
          return Ok(Self::Percentage(percent, base));
        }

        // Try parsing as fraction symbolic
        if let Some((numerator, denominator)) = fraction_symbolic(words) {
          return Ok(Self::Fraction(numerator, "/".to_string(), denominator));
        }
  ...
  pub fn as_f64(&self) -> f64 {
    match self {
      Self::Cardinal(v) => *v,
      Self::OrdinalSymbolic(v, _) | Self::Ordinal(v) => *v as f64,
      Self::PercentageSymbol(v, _) | Self::Percentage(v, _) => v / 100.0,
