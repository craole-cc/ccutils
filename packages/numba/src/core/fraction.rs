...
impl Fraction {
  ...
  fn numeric_to_fraction(numeric: f64, precision: usize) -> (isize, isize) {
    let denominator = 10_f64.powi(precision as i32) as isize; // Precision as denominator
    let numerator = (numeric * denominator as f64).round() as isize;
  ...
  fn fraction_to_words(numerator: isize, denominator: isize) -> String {
    let numerator_word = Self::number_to_words(numerator);
    let denominator_word = Self::denominator_to_words(denominator);
...
impl From<&str> for Fraction {
  fn from(symbolic: &str) -> Self {
    let (numerator, denominator) = Fraction::symbolic_to_fraction(symbolic);
    let numeric = numerator as f64 / denominator as f64;
    let worded = Fraction::fraction_to_words(numerator, denominator);
    Fraction {
      numeric,
      worded,
