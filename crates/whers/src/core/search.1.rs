...
use super::{
	utils::build_regex_search_input, FilterType, SearchBuilder,
};
...
impl Search {
	...
	pub(crate) fn new(
		...
		let regex_search_input = build_regex_search_input(
			search_input,
			file_ext,
			strict,
			ignore_case,
		...
		walker.filter_entry(move |entry| {
			filters.iter().all(|f| f.apply(entry, dirs))
		});
