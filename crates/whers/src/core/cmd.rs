...
fn check_command_type(command: &str) -> Option<String> {
...
pub fn pathof_cmd(
	command: &str,
) -> Result<HashSet<CommandLocation>, CommandError> {
	let mut locations = HashSet::new();

	// Check if it's a shell builtin
	if get_shell_builtin_commands().contains(&command.to_lowercase())
	{
		locations.insert(CommandLocation::ShellBuiltin);
	}

	// Use command -v to check for aliases, built-ins, or executables
	if let Some(command_type) = check_command_type(command) {
		// Check if it's an alias or a command (parse type or command -v result)
		if command_type.starts_with("alias ") {
			let alias_target = command_type
				.strip_prefix("alias ")
				.unwrap_or("")
				.trim();
			locations.insert(CommandLocation::ShellAlias(
				alias_target.to_string(),
			));
		} else {
			locations.insert(CommandLocation::Executable(
				PathBuf::from(command_type),
			));
		}
	}
	...
	if let Some(path) = whereismybin(command) {
		locations.insert(CommandLocation::Executable(path));
	}
