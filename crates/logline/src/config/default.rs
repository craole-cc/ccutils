...
impl Config {
  ...
  pub fn init(&mut self) {
    ...
    let mut subscriber = tracing_subscriber::fmt().with_env_filter(filter);
    ...
    match self.display.pretty {
      true => match self.time {
        Time::None => subscriber.without_time().pretty().init(),
        Time::Datetime => subscriber.pretty().init(),
        Time::Uptime => subscriber.with_timer(timer).pretty().init()
      },
  ...
  pub fn show_thread(mut self) -> Self {
    self.display.thread = true;
    self
  }

  pub fn hide_thread(mut self) -> Self {
    self.display.thread = false;
    self
  }

  pub fn show_thread_id(mut self) -> Self {
    self.display.thread_id = true;
    self
  }
}
