...
mod config_tests {
  use erks::{
    Code, ConfigError, Context, ErksContext, ErksError, ErksSeverity, Metadata
  };

  #[test]
  fn test_file_not_found_error() {
    let error = ConfigError::file_not_found("config.toml");
    assert_eq!(
      error.to_string(),
  ...
  fn test_invalid_format_error() {
    let error = ConfigError::invalid_format("config.toml", "Invalid syntax");
    assert_eq!(
      error.to_string(),
  ...
  fn test_missing_key_error() {
    let error = ConfigError::missing_key("database.url");
    assert_eq!(
      error.to_string(),
  ...
  fn test_invalid_value_error() {
    let error = ConfigError::invalid_value("server.port", "must be a number");
    assert_eq!(
      error.to_string(),
  ...
  fn test_generic_config_error() {
    ...
    let error = ConfigError::from(config_lib_error);
