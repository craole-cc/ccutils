...
/// and commas, then attempts to parse it as a `BigDecimal`. It handles both integer
...
pub fn float<T: AsRef<str>>(
	...
	let digits = if cleaned.starts_with(".") {
		...
		match BigInt::from_str(parts[0]) {
			Ok(digits) => digits,
			Err(err) => {
				return Err(Error::InvalidBigInt(err, cleaned.into()))
			}
		}
	};
	...
	let scale = match parts.get(1) {
		Some(fractional_part) if !fractional_part.is_empty() => {
			...
			let fractional_value = usize::from_str(fractional_part)
				.map_err(|err| {
				Error::InvalidFractional(
					err,
					fractional_part.to_string().into(),
				)
			})?;
