...
///
/// ```
/// let result = remove_commas_simd("1,234,567.89");
...
mod tests {
	...
	fn test_remove_commas() {
		assert_eq!(remove_commas_simd("1,234,567.89"), "1234567.89");
		assert_eq!(
			remove_commas_simd("no,commas,here"),
			"nocommashere"
		);
		assert_eq!(remove_commas_simd("123"), "123");
		assert_eq!(remove_commas_simd(""), "");
	}
}
