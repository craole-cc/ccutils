...
impl English {
	...
	fn float_to_cardinal(
		...
		if !integral_part.is_zero() {
			let integral_word =
				self.int_to_cardinal(integral_part)?;
...
impl Language for English {
	...
	fn to_year(&self, num: BigFloat) -> Result<String, Num2Err> {
			...
			|| high >= 100
		{
			...
			let high_word =
				self.int_to_cardinal(BigFloat::from(high))?;
	...
	fn to_currency(
		...
		if num.is_inf() {
			...
			let integral_part = num.int();
			let cents_nb = (num * BigFloat::from(100)).int()
				% BigFloat::from(100);
			let cents_words = self.int_to_cardinal(cents_nb)?;
			let cents_suffix =
				self.cents(currency, cents_nb != BigFloat::from(1));
			let integral_word =
				self.to_currency(integral_part, currency)?;
