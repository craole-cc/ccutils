...
/// and commas, then attempts to manually validate and parse it as a `BigDecimal`.
/// It supports both regular decimal notation and scientific notation (e.g., "1.23e4").
///
/// # Arguments
///
...
pub fn scientific_notation<T: AsRef<str>>(
	input: T,
	...
	let (mantissa_part, exponent_part) =
		match cleaned.find(|c: char| c == 'e' || c == 'E') {
			Some(index) => {
	...
	let mantissa =
		BigDecimal::from_str(mantissa_part).map_err(|err| {
			Error::InvalidMantissa(err, mantissa_part.into())
		})?;
	...
	let exponent = isize::from_str(exponent_str).map_err(|err| {
		Error::InvalidExponent(err, exponent_str.into())
	})?;

	// Multiply the mantissa by 10^exponent to get the final result
	let scale =
		BigInt::from_u64(10).unwrap().pow(exponent.abs() as u32);
