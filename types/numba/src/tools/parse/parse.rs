...
pub fn fraction_symbolic<S: AsRef<str>>(
	words: S,
) -> Option<(usize, usize)> {
	match words.as_ref() {
		"1/2" => Some((1, 2)),
...
pub fn fraction_worded<S: AsRef<str>>(
	words: S,
) -> Option<(usize, usize)> {
	match words.as_ref() {
		"one half" => Some((1, 2)),
...
pub fn percentage_symbol<S: AsRef<str>>(
	words: S,
) -> Option<(f64, usize)> {
...
pub fn percentage_worded<S: AsRef<str>>(
	words: S,
) -> Option<(f64, usize)> {
	let percent = match words.as_ref() {
