...
impl Numeral {
	pub fn fraction_from_float(value: f64) -> (usize, usize) {
		...
		let numerator =
			(fractional_part * denominator as f64).round() as usize;

		(whole_part * denominator + numerator, denominator)
	}

	pub fn update_representations(&mut self) {
		self.cardinal_worded =
			self.to_cardinal_worded(self.cardinal_symbol);
		...
		self.fraction_worded =
			self.to_fraction_worded(self.whole, self.fractional);
		self.percentage_symbol =
			format!("{:.1}%", self.cardinal_symbol * 100.0);
		self.percentage_worded =
			self.to_percentage_worded(self.cardinal_symbol);
