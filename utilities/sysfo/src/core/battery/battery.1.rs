        ...
        let status = match self.status {
            ...
            State::Full => {
                format!(
                    ...
                    self.status,
                    if has_cycles {
                        format!(", having being cycled {} times.", self.pretty_cycles().0,)
                    } else {
                        ".".to_string()
                    }
                )
...
impl Battery {
    pub fn handle_command(&self, command: Option<&BatteryCommands>) -> String {
        match command {
            None => self.all(),
            Some(cmd) => match cmd {
                ...
                BatteryCommands::Time => self.pretty_time_left(),
                BatteryCommands::Technology => self.pretty_technology(),
                BatteryCommands::Cycle => self.pretty_cycles().0.to_string(),
                BatteryCommands::Brand => self.pretty_brand().to_string(),
            },
        }
    }
}
