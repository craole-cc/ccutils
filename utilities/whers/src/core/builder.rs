use super::{utils::replace_tilde_with_home_dir, FilterType, Search};
...
impl SearchBuilder {
	...
	pub fn ext(mut self, ext: impl Into<String>) -> Self {
		...
		self.file_ext = Some(
			ext.strip_prefix('.').map_or(ext.clone(), str::to_owned),
		);
		self
	}
	...
	///     .collect();
	/// ```
	pub fn more_locations(
		mut self,
		more_locations: Vec<impl AsRef<Path>>,
	) -> Self {
...
impl Default for SearchBuilder {
	...
	fn default() -> Self {
		Self {
			search_location: std::env::current_dir()
