...
pub struct Search {
	...
	///
	/// If `Some(pattern)`, the search will match files and directories that match the specified pattern.
	/// If `None`, the search will match all files and directories (equivalent to the default pattern `.*`).
